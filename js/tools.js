...
function enableTool(tool) {
    ...
    currentImage.onerror = () => {
        console.error(`Failed to load image for tool: ${currentImage.src}`);
    };
}
...
const updateCursor = (angle, src, enlarge, offsetx, offsety, brushsize) => {
    ...
    cursorImage.crossOrigin = "anonymous"; // CORS 설정 추가
    cursorImage.src = src;
    ...
    cursorImage.onload = () => {
        ...
        const cursorDataURL = cursorCanvas.toDataURL('image/png'); // 에러 해결
        canvas.style.cursor = `url(${cursorDataURL}) ${brushSize / 2} ${brushSize / 2}, auto`;
    };

    cursorImage.onerror = () => {
        console.error(`Failed to load cursor image from ${src}`);
    };
};
...
let bubbleMouseDown = null;
let bubbleMouseMove = null;
let bubbleMouseUp = null;

// Updated removeBubbleListeners
